...
PC_qsar_model = function(
  ...
  PC_QSAR_blood_water = 0.008 *10^log_Kow + 0.007*(10^log_Kow)^0.94 + 0.134*(10^log_Kow)^0.63 + 0.851
  PC_blood_water = PC_QSAR_blood_water 
  PC_QSAR_liver = (0.037*(10^log_Kow) + 0.025*(10^log_Kow)^0.94+ 0.234*(10^log_Kow)^0.63 +0.704)/PC_blood_water
  PC_QSAR_gonads = (0.042*(10^log_Kow) + 0.024*(10^log_Kow)^0.94+ 0.262 *(10^log_Kow)^0.63 +0.672)/PC_blood_water
  PC_QSAR_brain = (0.047*(10^log_Kow) + 0.043*(10^log_Kow)^0.94+ 0.228 *(10^log_Kow)^0.63 +0.682)/PC_blood_water
  PC_QSAR_fat = (0.884*(10^log_Kow) + 0.009*(10^log_Kow)^0.94+ 0.048 *(10^log_Kow)^0.63 +0.059)/PC_blood_water 
  PC_QSAR_skin = (0.037*(10^log_Kow) + 0.012*(10^log_Kow)^0.94+ 0.259 *(10^log_Kow)^0.63 +0.692)/PC_blood_water
  PC_QSAR_GIT = (0.052*(10^log_Kow) + 0.023*(10^log_Kow)^0.94+ 0.255 *(10^log_Kow)^0.63 +0.67)/PC_blood_water
  PC_QSAR_kidney = (0.059*(10^log_Kow) + 0.023*(10^log_Kow)^0.94+ 0.215 *(10^log_Kow)^0.63+0.703)/PC_blood_water
  PC_QSAR_rp = (0.066*(10^log_Kow) + 0.008*(10^log_Kow)^0.94+ 0.338 *(10^log_Kow)^0.63 +0.587)/PC_blood_water
  PC_QSAR_pp = (0.024*(10^log_Kow) + 0.009*(10^log_Kow)^0.94+ 0.199*(10^log_Kow)^0.63 +0.769)/PC_blood_water
  Plivb = PC_QSAR_liver
...
output <- function(parms){
  ...
  PC_BPA = PC_qsar_model(log_Kow = log_Kow )
  parameters = c(
...
for (i in 1:number_of_rows) {
  ...
  if (all(sapply(row_values, is.numeric))) {
    ...
    excel_filename <- paste0("../results/", num, " ", species, " ", chemicals, " ", BW, " ", time_final_dose, " ", Dose_water, " setII_", current_time, ".xlsx")
