...
ZF.model <- function(t,initial_v, parameters) {
  with(as.list(c(initial_v, parameters)),{
    ...
    dA_ven = (dA_admin_gil-dA_excr_gil-Fcard*(A_ven/V_ven) #The venous compartment is where the chemical gets absorbed
...
PC_qsar_model = function(a_PC = 0.73,
                         b_PC = 0.88,
  ...
  PC_QSAR_fat = (water_fat + 10^(a_Bar*log_Kow + b_Bar + c_Bar * log10(lipids_fat) ))/PC_blood_water #2.55
  ...
  Prpb = PC_QSAR_rp
  Pppb = PC_QSAR_pp
  PBW = PC_blood_water*Funbound #adjust for unbound fraction
...
output <- function(parms){
  ...
  method = as.character("lsoda") # differential solving method
  start = parms[["start"]]
  stop = parms[["stop"]]
  Res_times = c(1
  )
  times = sort( unique( c(seq(start, as.numeric(stop), 0.1), Res_times)))
  ...
  water_liver = parms[["water_liver"]]
  water_brain = parms[["water_brain"]]
  water_gonads= parms[["water_gonads"]]
  water_fat = parms[["water_fat"]]
  water_skin = parms[["water_skin"]]
  water_GIT = parms[["water_GIT"]]
  water_kidney= parms[["water_kidney"]]
  water_rp = parms[["water_rp"]] 
  water_pp = parms[["water_pp"]]
  lipids_liver = parms[["lipids_liver"]]
  lipids_brain = parms[["lipids_brain"]]
  lipids_gonads= parms[["lipids_gonads"]]
  lipids_fat = parms[["lipids_fat"]]
  lipids_skin = parms[["lipids_skin"]]
  lipids_GIT = parms[["lipids_GIT"]]
  lipids_kidney= parms[["lipids_kidney"]]
  lipids_rp = parms[["lipids_rp"]] 
  lipids_pp = parms[["lipids_pp"]]
  ...
  PC_BPA = PC_qsar_model(log_Kow = log_Kow, Funbound = Unbound_fraction,
                         water_liver = water_liver , lipids_liver = lipids_liver ,
                         water_gonads= water_gonads , lipids_gonads = lipids_gonads,
                         water_brain = water_brain , lipids_brain = lipids_brain,
                         water_fat = water_fat , lipids_fat = lipids_fat,
                         water_skin = water_skin , lipids_skin = lipids_skin,
                         water_GIT = water_GIT , lipids_GIT = lipids_GIT,
                         water_kidney= water_kidney , lipids_kidney = lipids_kidney,
                         water_rp = water_rp , lipids_rp = lipids_rp,
                         water_pp = water_pp , lipids_pp = lipids_pp)
  ...
  parameters = c(
    ...
    Ke_urine = parms[["Ke_urine"]], 
    Ke_feces = parms[["Ke_feces"]], 
    Ke_bile = parms[["Ke_bile"]], #dummy variable 
    K_BG = parms[["K_BG"]], #dummy variable 
    urine_rate = parms[["urine_rate"]],
...
data_path <- "../batch_pipeline/11.xlsx"
...
for (i in 1:number_of_rows) {
  ...
  if (all(sapply(row_values, is.numeric))) {
    ...
    excel_filename <- paste0("../results/", num, " ", species, " ", chemicals, " ", BW, " ", time_final_dose, " ", Dose_water, " setI_", current_time, ".xlsx")
